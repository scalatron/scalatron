...
object Main {
  ...
  val CopyableExtensions =
    Iterable(".css", ".html", ".js", ".scala", ".java", ".png", ".gif", ".jpg")

  def main(args: Array[String]): Unit = {

    val (verbose: Boolean, inPath: String, outPath: String) =
      args.length match {
        case 1 =>
          println(
            "Syntax: java -jar ScalaMarkdown.jar [-verbose] inputPath outputPath")
          println("where:")
          println(
            "  - 'inputPath' is either a file or a directory containing markdown")
        ...
        case _ =>
          ...
          println(
            "Syntax: java -jar ScalaMarkdown.jar [-verbose] inputPath outputPath")
          println(
            "where inputPath is either a file or a directory and outputPath is a directory")
          System.exit(-1)
      }
    ...
    if (inFile.isDirectory) {
      ...
      processDirectory(inputBaseDirectoryPath,
                       outputDirectoryPath,
  ...
  /** Processes a directory full of files and sub-directories.
    ...
    */
  def processDirectory(inputDirectoryPath: String,
  ...
  /** Processes a single file, either copying it or markdown-converting it.
    ...
    * @param verbose if true, log verbose output
    */
  def processFile(inputFilePath: String,
                  outputDirectoryPath: String,
                  inputBaseDirectoryPath: String,
                  outputBaseDirectoryPath: String,
                  verbose: Boolean): Unit = {
    ...
    if (outputFilename.isEmpty) {
      ...
      if (outputFilename == inputFilename) {
        ...
        convertFileFromMarkdownToHtml(inputFilePath,
                                      outputFilePath,
                                      inputBaseDirectoryPath,
                                      outputBaseDirectoryPath)
      }
  ...
  def computeOutputFilename(inputFilename: String) =
    ...
    } else if (inputFilename.endsWith(".markdown")) {
      inputFilename.dropRight(9) + ".html"
    } else if (CopyableExtensions.exists(inputFilename.endsWith)) {
      inputFilename
    } else {
      ""
    }
  ...
  /** Convert a single file from Markdown to HTML.
    ...
    * @param outputBaseDirectoryPath the topmost output directory, stays constant during recursion
    */
  def convertFileFromMarkdownToHtml(inputFilePath: String,
    ...
    val (liquidParamMap, inputText): (Map[String, String], String) =
      if (inputLines.head.startsWith(LiquidMarker)) {
        var linesTaken = 0
        val map =
          inputLines.tail // drop starting "---"
            .takeWhile(line => {
              linesTaken += 1;
              line.contains(':') && !line.startsWith(LiquidMarker)
            }) // between Liquid markers ("---")
            .map(line => line.split(':'))
            .flatMap(array =>
              if (array.length == 2) Some((array(0).trim, array(1).trim))
              else None) // remove None values
            .toMap
      ...
      if (liquidParamMap.isEmpty) {
        ...
        val templateContentOpt = templateFilenameOpt match {
          case Some(templateFilename) =>
            ...
            try {
              ...
              case t: Throwable =>
                System.err.println(
                  s"error: template file not found: $templateFilePath")
                None
            }
          case None => None
        }
        ...
        val templatedContent = templateContentOpt match {
          case Some(templateContent) =>
            templateContent.replace(ContentMarker, convertedHtml)
