...
object Markdown {
  ...
  val rInsideTags = Pattern.compile(
    "<(/?" + htmlNameTokenExpr + "(?:\\s+(?:" +
        ...
        (Pattern.compile("--") -> dash) ::
          (Pattern.compile("\\(r\\)", Pattern.CASE_INSENSITIVE) -> reg) ::
            (Pattern.compile("\\(c\\)", Pattern.CASE_INSENSITIVE) -> copy) ::
              (Pattern
                .compile("\\(tm\\)", Pattern.CASE_INSENSITIVE) -> trademark) ::
...
// # Processing Stuff
case class MacroDefinition(pattern: String,
                           flags: String,
...
class MarkdownText(source: CharSequence) {
  ...
  protected def encodeAmpsAndLts(text: StringEx) =
    text.replaceAll(rEscAmp, "&amp;").replaceAll(rEscLt, "&lt;")
  ...
  protected def hashHtmlBlocks(text: StringEx): StringEx = {
    ...
    if (m.find) {
      ...
      inlineHtml
        .replaceAll(rInlineMd, m => new MarkdownText(m.group(1)).toHtml)
      // Hashify block
  ...
  protected def doHeaders(text: StringEx): StringEx =
    text
      .replaceAll(rH1, m => {
        ...
        val id = m.group(3)
        val idAttr =
          if (id == null) to_id(label.toString) else " id = \"" + id + "\""
        "<h1" + idAttr + ">" + label + "</h1>"
      })
      .replaceAll(rH2, m => {
        val label = runSpanGamut(new StringEx(m.group(1)))
      ...
      .replaceAll(rHeaders, m => {
        ...
        val id = m.group(4)
        val idAttr = if (id == null) { to_id(label.toString) } else {
          " id = \"" + id + "\""
        }
  ...
  def to_id(label: String) =
    " id = \"" + label.replaceAll("""[^a-zA-Z0-9\-:]""", "_") + "\""
  ...
  protected def doBlockQuotes(text: StringEx): StringEx =
    text.replaceAll(rBlockQuote, m => {
      val content = new StringEx(m.group(1)).replaceAll(rBlockQuoteTrims, "")
  ...
  /**
    ...
    */
  protected def formParagraphs(text: StringEx): StringEx =
    new StringEx(
      rParaSplit
        .split(text.toString.trim)
        .map(para =>
          htmlProtector.decode(para) match {
            case Some(d) => d
            case _ =>
              "<p>" + runSpanGamut(new StringEx(para)).toString + "</p>"
        })
        .mkString("\n\n"))
  ...
  /**
    * Process images.
    */
  protected def doImages(text: StringEx): StringEx =
    text.replaceAll(rImage, m => {
      val alt = m.group(1)
      val src = m.group(2)
      val title = m.group(4)
  ...
  /**
    ...
    */
  protected def doRefLinks(text: StringEx): StringEx =
    text.replaceAll(rRefLinks, m => {
      ...
      links.get(id) match {
        case Some(ld) =>
          val title =
            ld.title.replaceAll("\\*", "&#42;").replaceAll("_", "&#95;").trim
          val url = ld.url.replaceAll("\\*", "&#42;").replaceAll("_", "&#95;")
  ...
  protected def doInlineLinks(text: StringEx): StringEx =
    text.replaceAll(rInlineLinks, m => {
      ...
      val url = m.group(2).replaceAll("\\*", "&#42;").replaceAll("_", "&#95;")
      var titleAttr = ""
      val title = m.group(5)
      if (title != null)
  ...
  protected def doMacros(text: StringEx): StringEx = {
    macros.foldLeft(text)((t, m) => {
      t.replaceAllFunc(m.regex, m.replacement, m.literally)
    })
  }
...
class TOC(val start_level: Int, val end_level: Int, val html: String) {
  ...
  val headings: Seq[Heading] =
    TOC.rHeadings.findAllIn(html).matchData.toList.flatMap { m =>
      if (m.group(4) != null) {
  ...
  val toHtml: String =
    if (headings.isEmpty) ""
      ...
      def startList(l: Int) =
        sb.append(
          "  " * (1 + l - start_level) + """<li><ul style="list-style:none;">""" + "\n")
      def endList(l: Int) =
        sb.append("  " * (l - start_level) + "</ul></li>\n")
      def add(l: Int, h: Heading) =
        sb.append(
          "  " * (1 + l - start_level) + "<li>" + h.toString + "</li>\n")
      def formList(level: Int, index: Int): Unit =
