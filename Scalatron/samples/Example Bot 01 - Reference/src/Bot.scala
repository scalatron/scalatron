    ...
    if (dontFireAggressiveMissileUntil < bot.time && bot.energy > 100) { // fire attack missile?
      nearestEnemyMaster match {
        ...
        case Some(relPos) => // a master is nearby
          ...
          bot.set(
            "dontFireAggressiveMissileUntil" -> (bot.time + relPos.stepCount + 1))
      }
      ...
      nearestEnemySlave match {
        ...
        case Some(relPos) => // an enemy slave is nearby
          if (relPos.stepCount < 8) {
            ...
            bot.set(
              "dontFireDefensiveMissileUntil" -> (bot.time + relPos.stepCount + 1))
          }
      }
    }
  }
...
case class BotImpl(inputParams: Map[String, String]) extends MiniBot {
  ...
  def spawn(offset: XY, params: (String, Any)*) =
    append(
      "Spawn(direction=" + offset +
        (if (params.isEmpty) ""
         else "," + params.map(e => e._1 + "=" + e._2).mkString(",")) +
        ")")
...
case class XY(x: Int, y: Int) {
  ...
  def stepsTo(pos: XY): Int =
    (this - pos).stepCount // steps to reach pos: max delta X or Y
  def stepCount: Int =
    x.abs.max(y.abs) // steps from (0,0) to get here: max X or Y
        ...
        unit.y match {
          ...
          case 0 =>
            throw new IllegalArgumentException(
              "cannot compute direction index for (0,0)")
  ...
  def wrap(boardSize: XY) = {
    val fixedX =
      if (x < 0) boardSize.x + x
      else if (x >= boardSize.x) x - boardSize.x
      else x
    val fixedY =
      if (y < 0) boardSize.y + y
      else if (y >= boardSize.y) y - boardSize.y
      else y
