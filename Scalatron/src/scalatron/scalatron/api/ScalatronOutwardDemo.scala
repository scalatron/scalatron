...
object ScalatronOutwardDemo {
  def demo(args: Array[String]): Unit = {
    ...
    try {
      ...
      // create a web user
      scalatron.createUser(name = "ExampleUser",
                           password = "",
      ...
      assert(
        new File(usersBaseDirPath + "/ExampleUser/src/Bot.scala").exists())
      ...
      assert(
        new File(usersBaseDirPath + "/ExampleUser/bot/ScalatronBot.jar")
          .exists())
      ...
      user.publish()
      assert(
        new File(pluginBaseDirPath + "/ExampleUser/ScalatronBot.jar").exists())
      ...
      FileUtil
        .deleteRecursively(tmpDirPath, atThisLevel = true, verbose = false)
    }
  }
}
