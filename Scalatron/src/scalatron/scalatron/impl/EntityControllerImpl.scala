...
case class EntityControllerImpl(name: String,
                                controlFunction: String => String)
    extends EntityController {
  def respond(input: String) = controlFunction(input)
  def withReplacedControlFunction(controlFunction: (String) => String) =
    copy(controlFunction = controlFunction)
}

object EntityControllerImpl {
  def fromPlugins(plugins: Iterable[Plugin])(
      implicit executionContextForUntrustedCode: ExecutionContext)
    : Iterable[EntityController] = {
    ...
    val future = Future.traverse(plugins)(plugin =>
      Future {
        try {
          ...
          case t: Throwable =>
            System.err.println(
              "error: exception while instantiating control function of plugin '" + plugin.name + "': " + t)
            None
        }
    })
    ...
    val pluginsAndControlFunctions =
      try {
        ...
        case t: TimeoutException =>
          System.err.println(
            "warning: timeout while instantiating control function of one of the plugins")
          Iterable.empty // temporary - disables ALL bots, which is not the intention
      }

    pluginsAndControlFunctions.map(pair =>
      EntityControllerImpl(pair._1.name, pair._2): EntityController)
