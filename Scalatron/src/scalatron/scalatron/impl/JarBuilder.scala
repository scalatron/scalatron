...
object JarBuilder {
  ...
  /** Given an input directory containing other directories which may include Java .class
    ...
    */
  def apply(inputDirectoryPath: String,
            jarFilePath: String,
    ...
    if (!rootDirectory.exists) {
      ...
      val canonicalRootPath =
        canonizeSlashesInPath(rootPath, asDirectory = true)
      ...
      val fileOutputStream =
        try {
          ...
          case e: Throwable =>
            println(
              "error: failed to open file output stream on: " + jarFilePath + ": " + e)
            return false
        }
      ...
      try {
        if (verbose)
          println(
            "      building jar file '" + jarFilePath + "' from class files in '" + inputDirectoryPath + "'...")
        ...
        case e: Throwable =>
          System.err.println(
            "error: failed to build jar file: " + jarFilePath + ": " + e)
          return false
      } finally {
  ...
  private def add(source: File,
    ...
    if (source.isDirectory) {
      if (!relativeName.isEmpty) {
        val adjustedRelativeName =
          if (relativeName.endsWith("/")) relativeName else relativeName + "/"
        if (verbose)
          println("        adding directory: " + adjustedRelativeName)
      ...
      if (nestedFiles != null) {
        nestedFiles.foreach(nestedFile =>
          add(nestedFile, relativeTo, jarOutputStream, verbose))
      }
    } else {
      val relativeName = name.drop(relativeTo.length)
