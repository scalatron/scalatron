...
object Plugin {
  ...
  /** Plugin.FromJarFile: a plugin implementation that provides a control function that was loaded from a
    ...
    */
  case class FromJarFile(dirPath: String,
                         filePath: String,
                         fileTime: Long,
                         name: String,
  ...
  /** Plugin.LoadFailure: Container used to report a plug-in load failure.
    ...
    */
  case class LoadFailure(dirPath: String,
                         filePath: String,
                         fileTime: Long,
                         exception: Throwable)
      extends DiskInfo {
  ...
  def loadBotControlFunctionFrom(
    ...
    if (verbose)
      println(
        s"info: will attempt to load bot plug-in '${jarFile.getAbsolutePath}'...")
    loadClassAndMethodFromJar(
      jarFile,
      fullyQualifiedControlFunctionFactoryClassNamesToTry,
      "create",
      None,
      verbose) match {
      case Right(throwable) =>
        ...
        loadClassAndMethodFromJar(jarFile,
                                  fullyQualifiedControlFunctionClassNamesToTry,
                                  "respond",
                                  Some(classOf[String]),
                                  verbose) match {
          case Right(throwable2) =>
            ...
            if (verbose)
              System.err.println(
                s"    error: failed to load a control function factory or control function from plug-in '${jarFile.getAbsolutePath}': ${throwable2.toString}")
            Right(throwable2)
          ...
          case Left((extractedClass, methodOnExtractedClass)) =>
            ...
            if (verbose)
              println(
                s"    info: ControlFunction class and method located in bot plug-in '${jarFile.getAbsolutePath}', will try to instantiate control function...")
            try {
              val classInstance: Any = extractedClass.newInstance()
              val controlFunction: String => String = (input: String) =>
                methodOnExtractedClass
                  .invoke(classInstance, input)
                  .asInstanceOf[String]
              val controlFunctionFactory: () => (String => String) = () =>
                controlFunction
              if (verbose)
                println(
                  s"    info: successfully extracted control function from bot plug-in '${jarFile.getAbsolutePath}'...")
              ...
              case t: Throwable =>
                ...
                if (verbose)
                  System.err.println(
                    s"    error: failed to extract control function factory from bot plug-in '${jarFile.getAbsolutePath}': ${t.toString}")
                Right(t)
            }
        }
      ...
      case Left((extractedClass, methodOnExtractedClass)) =>
        ...
        if (verbose)
          println(
            s"    info: ControlFunctionFactory class and method located in bot plug-in '${jarFile.getAbsolutePath}', will try to instantiate control function factory...")
        try {
          val classInstance: Any = extractedClass.newInstance()
          val factoryFunction: () => (String => String) = () =>
            methodOnExtractedClass
              .invoke(classInstance)
              .asInstanceOf[(String => String)]
          if (verbose)
            println(
              s"    info: successfully extracted control function factory from bot plug-in '${jarFile.getAbsolutePath}'...")
  ...
  def loadClassAndMethodFromJar(
      ...
      verbose: Boolean)
    : Either[(Class[_], java.lang.reflect.Method), Throwable] = {
    try {
      ...
      while (iterator.hasNext) {
        ...
        try {
          if (verbose)
            println(
              s"    info: will try to load class '$fullyQualifiedClassName' from plug-in '$pluginFilePath'...")
          val extractedClass =
            Class.forName(fullyQualifiedClassName, true, classLoader)

          if (verbose)
            println(
              s"    info: class '$fullyQualifiedClassName' loaded, will try to find method '$methodName'...")
          val methodOnExtractedClass = methodParameterClass match {
            case None => extractedClass.getMethod(methodName)
            case Some(parameterClass) =>
              extractedClass.getMethod(methodName, parameterClass)
          }

          if (verbose)
            println(
              s"    info: successfully located method '$methodName' on class '$fullyQualifiedClassName' in plug-in '$pluginFilePath'...")
          ...
          case t: Throwable =>
            ...
            if (verbose)
              println(
                s"    info: failed to load class '$fullyQualifiedClassName' from plug-in '$pluginFilePath'...: ${t.toString}")
        }
      }
      ...
      lastError match {
        case Some(t) => Right(t)
        case None =>
          Right(new IllegalStateException(
            s"none of the factory class candidates found in '$pluginFilePath': ${fullyQualifiedClassNamesToTry
              .mkString(", ")}"))
      }
    } catch {
