...
case class PluginCollection(
  ...
  def plugins: Iterable[Plugin.FromJarFile] =
    loadResults.filter(_.isLeft).map(_.left.get)

  def loadFailures: Iterable[Plugin.LoadFailure] =
    loadResults.filter(_.isRight).map(_.right.get)
  ...
  def incrementalRescan: PluginCollection = {
    ...
    /** Searches the existing list of load results for an entry with the given plug-in file
      ...
      * recyclable Either.
      */
    def existingRecyclableLoadResult(pluginFilePath: String, fileTime: Long)
      : Option[Either[Plugin.FromJarFile, Plugin.LoadFailure]] =
      loadResults.find {
        case Left(externalPlugin) =>
          externalPlugin.filePath == pluginFilePath && externalPlugin.fileTime == fileTime
        case Right(loadFailure) =>
          loadFailure.filePath == pluginFilePath && loadFailure.fileTime == fileTime
      }
        ...
        filteredPluginDirectories.map(pluginDirectory => {
          ...
          try {
            if (pluginJarFile.exists()) {
                ...
                existingRecyclableLoadResult(pluginJarFilePath, fileTime).getOrElse {
                  ...
                  eitherFactoryOrException match {
                    case Left(controlFunctionFactory) =>
                      val externalPlugin =
                        Plugin.FromJarFile(pluginDirectoryPath,
                                           pluginJarFilePath,
                                           fileTime,
                    ...
                    case Right(exception) =>
                      val loadFailure = Plugin.LoadFailure(pluginDirectoryPath,
                                                           pluginJarFilePath,
                                                           fileTime,
                                                           exception)
              ...
              println(
                "warning: plug-in file does not exist: " + pluginJarFilePath)
              None
            }
          } catch {
            case t: Throwable =>
              System.err.println(
                "warning: exception while examining plug-in file: " + pluginJarFilePath)
              None
          }
        })

      PluginCollection(pluginDirectoryPath,
                       gameSpecificPackagePath,
                       verbose,
                       options.flatten)
    }
  ...
  def fullRescan: PluginCollection = {
    println(
      "Rescanning from scratch for plugins in '" + pluginDirectoryPath + "'...")
    ...
    if (pluginDirectories == null) {
      println(
        "Plugin parent directory not valid: '" + pluginDirectoryPath + "'")
      PluginCollection(pluginDirectoryPath, gameSpecificPackagePath, verbose)
    } else {
      // filter out directories like ".info"
      val filteredPluginDirectories =
        ...
        filteredPluginDirectories.map(pluginDirectory => {
          ...
          try {
            ...
            if (pluginFile.exists()) {
              ...
              System.err.println(
                "warning: plug-in file does not exist: " + pluginFilePath)
              None
            }
          } catch {
            case t: Throwable =>
              System.err.println(
                "warning: exception while examining plug-in file: " + pluginFilePath)
              None
          }
        })
...
object PluginCollection {
  def apply(pluginDirectoryPath: String,
            gameSpecificPackagePath: String,
            verbose: Boolean): PluginCollection =
    PluginCollection(pluginDirectoryPath,
                     gameSpecificPackagePath,
                     verbose,
                     Iterable.empty)
