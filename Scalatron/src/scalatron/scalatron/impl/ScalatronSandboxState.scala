...
case class ScalatronSandboxState(sandbox: ScalatronSandbox,
                                 simState: Simulation.UntypedState)
    extends Scalatron.SandboxState {
  ...
  def step(count: Int): SandboxState = {
    ...
    val executionContextForUntrustedCode =
      sandbox.user.scalatron.executionContextForUntrustedCode
  ...
  def entities =
    simState
      .entitiesOfPlayer(sandbox.user.name)
      .map(e => ScalatronSandboxEntity(e, this))
}
