...
object ConfigFile {
  ...
  def updateConfigFile(absolutePath: String,
                       key: String,
                       value: String): Unit = {
  ...
  /** Loads, parses, updates and writes back a file with one key/val pair per line. */
  def updateConfigFileMulti(absolutePath: String,
                            kvMap: Map[String, String]): Unit = {
    val updatedMap =
    ...
    use(new FileWriter(absolutePath)) { fileWriter =>
