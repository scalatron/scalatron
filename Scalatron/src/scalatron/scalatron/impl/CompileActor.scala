...
object CompileActor {
  ...
  /** Returns a collection of one or more class paths to be appended to the classPath property of the
    ...
    */
  private def detectScalaClassPaths(verbose: Boolean, cp: String): Iterable[String] = {
  ...
  // constants
  object Constants {
    val ScalaCompilerInfo = 0
    // see scala.tools.nsc.reporters.Reporter.INFO
    val ScalaCompilerWarning = 1
    // see scala.tools.nsc.reporters.Reporter.WARNING
...
case class CompileActor(verbose: Boolean) extends Actor {
  ...
  override def preStart() {
    ...
    //      val detectedScalaClassPaths = CompileActor.detectScalaClassPaths(true)//verbose)
    ...
    settings.classpath.value = scalatronJarFilePath
    settings.bootclasspath.value = scalaCompilerClassPath
    settings.bootclasspath.append(scalaLibraryClassPath)
