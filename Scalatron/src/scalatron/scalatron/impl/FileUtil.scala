      ...
      if (itemAtPath.isDirectory) {
        ...
        if (atThisLevel) {
          ...
          if (!itemAtPath.delete()) {
            System.err.println(
              s"error: failed to delete directory: ${itemAtPath.getAbsolutePath}")
            throw new IllegalStateException(
              "failed to delete directory at: " + itemAtPath.getAbsolutePath)
          }
        }
      } else {
        if (atThisLevel) {
          ...
          if (!itemAtPath.delete()) {
            System.err.println(
              s"error: failed to delete file: ${itemAtPath.getAbsolutePath}")
            throw new IllegalStateException(
              "failed to delete file: " + itemAtPath.getAbsolutePath)
          }
        }
      }
    }
