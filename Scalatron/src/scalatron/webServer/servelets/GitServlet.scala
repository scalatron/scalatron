...
case class GitServlet(context: WebContext)
  ...
  def getUser(req: HttpServletRequest) =
    Option(req.getHeader("authorization")) flatMap { authorization =>
      val decoded = new String(
        DatatypeConverter.parseBase64Binary(authorization.split(" ")(1)))
        .split(":", 2)
      val userName = decoded(0)
      val password = decoded(1)
      for (user <- scalatron.user(userName);
           if user.getPasswordOpt.contains(password)) yield user
    }
  ...
  class UserRepositoryResolver extends RepositoryResolver[HttpServletRequest] {
    override def open(req: HttpServletRequest, name: String) = {
      getUser(req)
        .map(_.asInstanceOf[ScalatronUser].gitRepository)
        .getOrElse(throw new RepositoryNotFoundException(name))
    }
  }
  ...
  class Receive extends ReceivePackFactory[HttpServletRequest] {
    ...
    class ReceiveCommits(user: User) extends PostReceiveHook {
      ...
      def onPostReceive(rp: ReceivePack,
                        commands: JCollection[ReceiveCommand]): Unit = {
        resetHead(rp)
        val ok =
          commands.asScala.forall(_.getResult == ReceiveCommand.Result.OK)
        if (ok && build(rp)) {
          List("", "Your Scalatron bot has been built successfully", "")
            .foreach(rp.sendMessage)
        }
      }
      ...
      def build(rp: ReceivePack) = {
        ...
        def getMessages =
          buildResult.messages.map(message =>
            s"${message.sourceFile}:${message.lineAndColumn} ${message.multiLineMessage}")
      ...
      def resetHead(rp: ReceivePack) =
        new Git(rp.getRepository).reset
