...
trait BaseServlet extends HttpServlet {
  ...
  def loadRelTextFile(relativePath: String) =
    FileUtil.loadTextFileContents(relativeToAbsolutePath(relativePath))

  def relativeToAbsolutePath(relativePath: String) = {
    if (relativePath.contains(".."))
      throw new IllegalAccessError(
        "error: illegal access attempt via relative path: " + relativePath)
  ...
  /** Replacement strings: $Error$, $ReturnUrl$, $ReturnUrlText$,
    */
  def serveErrorPage(errorString: String,
                     returnUrl: String,
                     returnUrlText: String,
                     request: HttpServletRequest,
                     response: HttpServletResponse): Unit = {
    serveString(loadRelTextFile("error.html")
  ...
  def streamFile(absolutePath: String,
                 mimeType: String,
                 request: HttpServletRequest,
                 response: HttpServletResponse): Unit = {
    ...
    try {
      ...
      case t: Throwable =>
        System.err.println(
          "error: failed to serve file '" + absolutePath + "': " + t)
