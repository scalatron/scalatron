...
case class HomePageServlet(context: WebContext) extends BaseServlet {
  override def doGet(req: HttpServletRequest,
    ...
    if (req.getRequestURI == "" || req.getRequestURI == "/") {
      ...
      val result =
        loadRelTextFile("index.html").replace("$WebUserList$", webUserListHtml)
  ...
  def doGetOther(request: HttpServletRequest,
                 response: HttpServletResponse): Unit = {
    val absolutePath = relativeToAbsolutePath(request.getRequestURI)
    val fileName = new File(absolutePath).getName

    if (fileName.endsWith(".html"))
      streamFile(absolutePath, "text/html", request, response)
    else if (fileName.endsWith(".css"))
      streamFile(absolutePath, "text/css", request, response)
    else if (fileName.endsWith(".ico"))
      streamFile(absolutePath, "image/ico", request, response)
      ...
      streamFile(absolutePath, "application/processing", request, response)
    else if (fileName.endsWith(".gif"))
      streamFile(absolutePath, "image/gif", request, response)
    else if (fileName.endsWith(".scala"))
      streamFile(absolutePath, "text/scala", request, response)
    else {
