...
case class AdminServlet(context: WebContext) extends BaseServlet {
  override def doGet(request: HttpServletRequest,
    ...
    if (!target.startsWith("/admin/")) {
      serveErrorPage("unexpected admin url: '" + target + "'",
                     request,
                     response)
    ...
    task match {
      ...
      case _ =>
        serveErrorPage("malformed admin url: '" + target + "'",
                       request,
                       response)
  ...
  // "/admin/list"
  private def handleAdminListPage(request: HttpServletRequest,
                                  response: HttpServletResponse): Unit = {
    ...
    val result =
      loadRelTextFile("admin.html").replace("$WebUserList$", webUserListHtml)
    serveString(result, request, response)
  }

  // "/admin/createUser?username=xxx"
  private def handleAdminCreateUserPage(
      request: HttpServletRequest,
      response: HttpServletResponse): Unit = {
    val userName = request.getParameter("username")
    if (userName == null || userName.isEmpty) {
      serveErrorPage("user name must not be empty",
                     "/admin/list",
    ...
    if (!context.scalatron.isUserNameValid(userName)) {
      serveErrorPage(
        "invalid user name: '" + userName + "': must not contain special characters",
        "/admin/list",
        "return to administration main page",
        request,
        response)
      System.err.println(
        "error: received invalid user name: '" + userName + "'")
      return
    }
    ...
    if (password1 == null || password2 == null) {
      serveErrorPage("invalid password field",
                     "/admin/list",
                     "return to administration main page",
                     request,
                     response)
    ...
    if (password1 != password2) {
      serveErrorPage("passwords fields do not match",
                     "/admin/list",
                     "return to administration main page",
                     request,
                     response)
    ...
    try {
      ...
      userOpt.foreach { user =>
        serveErrorPage("user already exists: '" + userName + "'",
                       "/admin/list",
                       "return to administration main page",
                       request,
                       response)
      ...
      case t: Throwable =>
        serveErrorPage(
          "unable to verify user directory for '" + userName + "'",
          "/admin/list",
          "return to administration main page",
          request,
          response)
        System.err.println(
          "error: failed to check whether user exists: '" + userName + "': " + t)
        return
    }

    // create the user password config file
    try {
      context.scalatron.createUser(userName,
                                   password1,
                                   SourceFileCollection.initial(userName))
    } catch {
      case t: Throwable =>
        serveErrorPage(
          "unable to write user configuration file for '" + userName + "'",
          "/admin/list",
          "return to administration main page",
          request,
          response)
        System.err.println(
          "error: unable to create user account for: '" + userName + "': " + t)
        return
    }
  ...
  private def handleAdminDeleteUserPage(
      request: HttpServletRequest,
      response: HttpServletResponse): Unit = {
    val userName = request.getParameter("username")
    if (userName == null || userName.isEmpty || !context.scalatron
          .isUserNameValid(userName)) {
      serveErrorPage("invalid user name: '" + userName + "'",
                     "/admin/list",
                     "return to administration main page",
                     request,
                     response)
    ...
    userOpt match {
      ...
      case Some(user) =>
        if (user.isAdministrator) {
          serveErrorPage(
            "deleting the 'Administrator' account is not permitted",
            "/admin/list",
            "return to administration main page",
            request,
            response)
          System.err.println(
            "error: deleting the 'Administrator' account is not permitted")
          return
        }
        ...
        try {
          ...
          case t: Throwable =>
            serveErrorPage(
              ...
              response)
            System.err.println(
              "error: failed to delete user: '" + userName + "': " + t)
            return
        }
  ...
  private def handleAdminSetPasswordPage(
      request: HttpServletRequest,
      response: HttpServletResponse): Unit = {
    val userName = request.getParameter("username")
    if (userName == null || userName.isEmpty || !context.scalatron
    ...
    val result =
      loadRelTextFile("setpassword.html").replace("$UserName$", userName)
    serveString(result, request, response)
  }

  // "/admin/updatePassword?username=xxx&password1=xxx&password2=xxx"
  private def handleAdminUpdatePasswordPage(
      request: HttpServletRequest,
      response: HttpServletResponse): Unit = {
    val userName = request.getParameter("username")
    if (userName == null || userName.isEmpty || !context.scalatron
    ...
    try {
      ...
      case t: Throwable =>
        serveErrorPage(
          ...
          response)
        System.err.println(
          "error: unable to update user password for: '" + userName + "': " + t)
        return
    }
