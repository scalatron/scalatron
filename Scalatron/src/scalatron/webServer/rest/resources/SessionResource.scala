...
class SessionResource extends ResourceWithUser {
  @POST
  def logon(@PathParam("user") userName: String,
            password: SessionResource.Password) = {
        ...
        user.getPasswordOpt match {
          case Some(pwd) =>
            if (pwd == password.getPassword) {
              ...
              System.err.println(
                "Refused logon for user '" + userName + "': wrong password")
              Response
                .status(
                  CustomStatusType(
          ...
          case None =>
            System.err.println(
              "Refused logon for user '" + userName + "': password configuration invalid")
            Response
              .status(CustomStatusType(
                HttpStatus.INTERNAL_SERVER_ERROR_500,
                "password configuration invalid for user '" + userName + "'"))
              .build()
        }
      case None =>
        System.err.println(
          "Refused logon for user '" + userName + "': unknown account")
        Response
          .status(
  ...
  def logoff(@PathParam("user") userName: String): Unit = {
    if (!userSession.isLoggedOnAsUser(userName)) {
      System.err.println(
        "Refused logout: not logged on as user '" + userName + "'")
      Response
        .status(
          CustomStatusType(HttpStatus.UNAUTHORIZED_401,
                           "not logged on as user '" + userName + "'"))
        .build()
    } else {
      userSession.destroy()
...
object SessionResource {
  def resources(userName: String) =
    Array(
      ...
      SessionResource.ResourceLink("Build",
                                   s"/api/users/$userName/sources/build"),
      SessionResource.ResourceLink("Sandboxes",
                                   s"/api/users/$userName/sandboxes"),
      SessionResource
        .ResourceLink("Publish", s"/api/users/$userName/unpublished/publish"),
      SessionResource.ResourceLink("Published",
                                   s"/api/users/$userName/published"),
      SessionResource.ResourceLink("Unpublished",
                                   s"/api/users/$userName/unpublished"),
      SessionResource.ResourceLink("Versions",
                                   s"/api/users/$userName/versions")
    )
