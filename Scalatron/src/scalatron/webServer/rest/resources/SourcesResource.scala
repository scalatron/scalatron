      ...
      try {
        scalatron.user(userName) match {
          case Some(user) =>
            ...
            val versionLabel =
              if (sourceFileUpdate.versionLabel == null) ""
              else sourceFileUpdate.versionLabel
...
object SourcesResource {
  ...
  def versionPolicyFrom(s: String) =
    if (s == null || s.compareToIgnoreCase(
          SourcesResource.Constants.IfDifferent) == 0) {
      VersionPolicy.IfDifferent
    } else if (s.compareToIgnoreCase(SourcesResource.Constants.Always) == 0) {
      VersionPolicy.Always
    } else if (s.compareToIgnoreCase(SourcesResource.Constants.Never) == 0) {
      VersionPolicy.Never
    } else {
  ...
  case class SourceFileUpdate(var fileList: Array[SourceFile],
    ...
    def setVersionPolicy(versionPolicy: String): Unit = {
      this.versionPolicy = versionPolicy
    }

    def getVersionLabel = versionLabel
    def setVersionLabel(versionLabel: String): Unit = {
