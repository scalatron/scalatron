...
class SamplesResource extends Resource {
  ...
  def listSamples = {
    if (!userSession.isLoggedOnAsAnyone) {
      Response
        .status(
          CustomStatusType(HttpStatus.UNAUTHORIZED_401, "must be logged on"))
        .build()
    } else {
      /*
               {
  ...
  def createSample(sources: SamplesResource.CreateSample) = {
    if (!userSession.isLoggedOnAsAnyone) {
      ...
      try {
        ...
        val sourceFiles = sources.getFiles.map(sf =>
          Scalatron.SourceFile(sf.getFilename, sf.getCode))
        /*val sample =*/
        scalatron.createSample(sampleName, sourceFiles)
        val encodedSampleName = URLEncoder.encode(sampleName, "UTF-8")

        Response
          .created(
            URI.create(uriInfo.getAbsolutePath + "/" + encodedSampleName))
          .build()
      } catch {
            ...
            Response
              .status(
                CustomStatusType(
                  ...
                  s"sample '$decodedSampleName' ($sampleName) does not exist"))
              .build()
        }
      } catch {
      ...
      Response
        .status(
          CustomStatusType(
            ...
            s"must be logged on as '${Scalatron.Constants.AdminUserName}'"))
        .build()
    } else {
      try {
        val decodedSampleName = URLDecoder.decode(sampleName, "UTF-8")
