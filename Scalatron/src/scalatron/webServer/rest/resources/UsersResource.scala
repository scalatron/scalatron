...
class UsersResource extends Resource {
    ...
    if (!userSession.isLoggedOnAsAdministrator) {
      Response
        .status(CustomStatusType(
          ...
          "must be logged on as '" + Scalatron.Constants.AdminUserName + "'"))
        .build()
    } else {
      try {
        val user = scalatron.createUser(
          param.getName,
          param.getPassword,
          SourceFileCollection.initial(param.getName))
        ...
        case e: ScalatronException.IllegalUserName =>
          Response
            .status(CustomStatusType(HttpStatus.BAD_REQUEST_400, e.getMessage))
            .build()
        case e: ScalatronException.Exists =>
          Response
            .status(CustomStatusType(HttpStatus.FORBIDDEN_403, e.getMessage))
            .build()
            ...
            user.getAttributeMapOpt match {
                ...
                Response
                  .status(
                    CustomStatusType(
                      ...
                      "configuration attributes of user '" + userName + "' are invalid"))
                  .build()
              ...
              case Some(map) =>
                val attributes =
                  map
                    .filter(entry =>
                      entry._1 != Scalatron.Constants.PasswordKey) // don't serve the password
                    .map(entry =>
                      new UsersResource.Attribute(entry._1, entry._2))
                    .toArray
  ...
  @PUT
  @Path("{user}")
  def updateUserAttributes(attributeList: UsersResource.Attributes,
...
object UsersResource {
  ...
  case class AttributesAndResources(a: Array[Attribute],
                                    r: Array[SessionResource.ResourceLink]) {
    def getAttributes = a
    def getResources = r
  }
