...
class SandboxesResource extends ResourceWithUser {
    ...
    if (!userSession.isLoggedOnAsUserOrAdministrator(userName)) {
      Response
        .status(CustomStatusType(
          ...
          "must be logged on as '" + userName + "' or '" + Scalatron.Constants.AdminUserName + "'"))
        .build()
    } else {
      try {
        scalatron.user(userName) match {
          case Some(user) =>
            ...
            Response
              .ok(SandboxesResource.createSandboxResult(userName, state))
              .build()

          case None =>
            Response
              .status(
                CustomStatusType(HttpStatus.NOT_FOUND_404,
        ...
        case e: IllegalStateException =>
          ...
          Response
            .status(
              CustomStatusType(HttpStatus.INTERNAL_SERVER_ERROR_500,
                               e.getMessage))
            .build()
      }
    }
  ...
  @GET
  @Path("{id}/{time}")
  def getForIdAndTime(@PathParam("id") id: Int,
                      @PathParam("time") time: Int): Response = {
    if (!userSession.isLoggedOnAsUserOrAdministrator(userName)) {
      Response
        .status(CustomStatusType(
          HttpStatus.UNAUTHORIZED_401,
            ...
            case None =>
              ...
              Response
                .status(CustomStatusType(
                  ...
                  s"sandbox id=$id at time=$time for user '$userName' does not exist"))
                .build()
            ...
            case Some(currentSandboxState: SandboxState) =>
              ...
              if (currentId != id) {
                Response
                  .status(CustomStatusType(
                    HttpStatus.NOT_FOUND_404,
                    s"sandbox with id=$id does not exist for user '$userName'"))
                  .build()
              } else {
                ...
                if (timeDelta == 0) {
                  Response
                    .ok(
                      SandboxesResource
                        .createSandboxResult(userName, currentSandboxState))
                    .build()
                } else if (timeDelta < 0) {
                  Response
                    .status(CustomStatusType(
                      HttpStatus.NOT_FOUND_404,
                      s"cannot step sandbox with id=$id for user '$userName' backwards in time $timeDelta"))
                    .build()
                } else {
                  ...
                  Response
                    .ok(
                      SandboxesResource
                        .createSandboxResult(userName, updatedSandboxState))
                    .build()
                }
              }
            case _ =>
      ...
      case Some(sandbox: SandboxState) =>
        ...
        val mappedEntities = entities
          .map(e => {
            ...
            SandboxesResource.EntityDto(e.id,
                                        e.name,
                                        e.isMaster,
                                        SandboxesResource.InputCommand(
                                          opcode,
                                          JavaConverters.mapAsJavaMap(params)),
                                        SandboxesResource.extractOutput(
                                          e.mostRecentControlFunctionOutput),
                                        e.debugOutput)
          })
          .toArray
...
object SandboxesResource {
  ...
  object CommandParser {
    ...
    def splitCommandIntoOpcodeAndParameters(
        command: String): (String, Map[String, String]) = {
  ...
  private def createSandboxResult(userName: String,
                                  state: Scalatron.SandboxState)
    ...
    val timePlus1 =
      s"/api/users/$userName/sandboxes/$sandboxId/${sandboxTime + 1}"
    val timePlus2 =
      s"/api/users/$userName/sandboxes/$sandboxId/${sandboxTime + 2}"
    val timePlus10 =
      s"/api/users/$userName/sandboxes/$sandboxId/${sandboxTime + 10}"

    val mappedEntities = state.entities
      .map(e => {
        val inputString = e.mostRecentControlFunctionInput
        val (opcode, params) =
          if (inputString.isEmpty) {
        ...
        SandboxesResource.EntityDto(
          ...
          e.name,
          e.isMaster,
          SandboxesResource.InputCommand(opcode,
                                         JavaConverters.mapAsJavaMap(params)),
  ...
  private def extractOutput(in: Iterable[(String, Iterable[(String, String)])])
    : Array[SandboxesResource.InputCommand] =
    in.map(e => {
        val op = e._1
        val params = e._2
        SandboxesResource
          .InputCommand(op, JavaConverters.mapAsJavaMap(params.toMap))
      })
      .toArray
  ...
  case class StartConfig(var config: java.util.HashMap[String, String]) {
    ...
    def setConfig(config: java.util.HashMap[String, String]): Unit = {
      this.config = config
    }
  }
  ...
  case class EntityDto(id: Int,
                       name: String,
                       master: Boolean,
                       input: InputCommand,
                       output: Array[InputCommand],
                       debugOut: String) {
    def getId = id
