...
/** The central init point for the Scalatron RESTful web API.
  */
case class RestApplication(scalatron: Scalatron, verbose: Boolean)
...
class ListProvider extends JacksonJsonProvider {
  ...
  override def isWriteable(c: Class[_],
    ...
    super.isWriteable(arrayListClass,
                      arrayListClass,
                      annotations,
...
class MapProvider extends JacksonJsonProvider {
  ...
  override def isWriteable(c: Class[_],
    ...
    classOf[Map[_, _]].isAssignableFrom(c) && super.isWriteable(
      mapClass,
      mapClass,
      annotations,
      MediaType.APPLICATION_JSON_TYPE)
...
/** This context is used to inject the scalatron instance into the resources.
  ...
  */
@Provider
class ScalatronProvider(scalatron: Scalatron)
    extends ContextResolver[Scalatron] {
  def getContext(cls: Class[_]) =
    if (cls.isInstance(scalatron)) scalatron else null
}
...
/** This context is used to inject a verbosity flag into the resources.
  ...
  */
@Provider
class VerbosityProvider(verbosity: Verbosity)
    extends ContextResolver[Verbosity] {
  def getContext(cls: Class[_]) =
    if (cls.isInstance(verbosity)) verbosity else null
}
