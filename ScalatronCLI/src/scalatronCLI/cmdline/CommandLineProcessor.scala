...
object CommandLineProcessor {
  def apply(args: Array[String]): Unit = {
    if (args.length == 0) {
      ...
      println("")
      println(
        "where [-key value] corresponds to one of the following key/value pairs:")
      println("   -verbose yes|no     print verbose output (default: no)")
      println(
        "   -api <string>       the relative path of the server api (default: " + ScalatronRemote.Constants.ApiEntryPoint + ")")
      println(
        "   -port <int>         the port the server is listening on (default: " + ScalatronRemote.Constants.DefaultPort + ")")
      println(
        "   -hostname <name>    the hostname of the server (default: localhost)")
      println(
        "   -user <name>        the user name to log on as (default: " + ScalatronRemote.Constants.AdminUserName + ")")
      println(
      ...
      println(
        " java -jar ScalatronCLI.jar -user Administrator -password a -cmd deleteAllUsers")
      println(
    ...
    val webServerPort = argMap
      .get("-port")
      .map(_.toInt)
      .getOrElse(ScalatronRemote.Constants.DefaultPort)
    val webServerApi =
      argMap.getOrElse("-api", ScalatronRemote.Constants.ApiEntryPoint)

    val connectionConfig =
      ConnectionConfig(webServerHostname, webServerPort, webServerApi, verbose)
        ...
        try {
          command match {
            ...
            case "setUserAttribute" =>
              cmd_setUserAttribute(connectionConfig, argMap)
            case "getUserAttribute" =>
              cmd_getUserAttribute(connectionConfig, argMap)
            ...
            case "restoreVersion" =>
              cmd_restoreVersion(connectionConfig, argMap)
          ...
          case t: Throwable =>
            System.err.println(
              "error: command '" + command + "' failed: " + t.toString)
            System.exit(-1)
        }
  ...
  def cmd_users(connectionConfig: ConnectionConfig): Unit = {
    ...
    if (connectionConfig.verbose)
      println("Users on Scalatron server '" + scalatron.hostname + "':")
  ...
  /** -command createUser          create new user
    ...
    * -newPassword string     the password for the new user (default: empty password)
    */
  def cmd_createUser(connectionConfig: ConnectionConfig,
                     argMap: Map[String, String]): Unit = {
    argMap.get("-targetUser") match {
      case None =>
        System.err.println(
          "error: command 'createUser' requires option '-targetUser'")
        System.exit(-1)
        ...
        doAsAdministrator(
          ...
          (scalatron: ScalatronRemote, users: ScalatronRemote.UserList) => {
            ...
            handleScalatronExceptionsFor {
              ...
              if (connectionConfig.verbose)
                println(
                  "Created user '" + user.name + "' on the Scalatron server '" + scalatron.hostname + "'")
            }
          }
        )
    }
  ...
  /** -command deleteUser          deletes an existing user (along with all content!); Administrator only
    * -targetUser name        the name of the user to delete (required)
    */
  def cmd_deleteUser(connectionConfig: ConnectionConfig,
                     argMap: Map[String, String]): Unit = {
    argMap.get("-targetUser") match {
      case None =>
        System.err.println(
          "error: command 'deleteUser' requires option '-targetUser'")
        System.exit(-1)
        ...
        doAsAdministrator(
          ...
          (scalatron: ScalatronRemote, users: ScalatronRemote.UserList) => {
            users.get(targetUser) match {
              case None =>
                System.err.println(
                  "error: user '" + targetUser + "' does not exist")
                System.exit(-1)
              case Some(user) =>
                // create the new user account (1 round-trip)
                handleScalatronExceptionsFor {
                  ...
                  if (connectionConfig.verbose)
                    println(
                      "Deleted user '" + user.name + "' on the Scalatron server '" + scalatron.hostname + "'")
                }
            }
          }
        )
  ...
  /** -command deleteAllUsers          deletes all existing users (along with all content!); Administrator only
    */
  def cmd_deleteAllUsers(connectionConfig: ConnectionConfig,
                         argMap: Map[String, String]): Unit = {
    doAsAdministrator(
      connectionConfig,
      argMap,
      (scalatron: ScalatronRemote, users: ScalatronRemote.UserList) => {
        if (connectionConfig.verbose)
          println(
            s"Deleting ${users.size} users on server '${scalatron.hostname}'...")
        users.foreach(user => {
  ...
  /** -command setUserAttribute       sets a configuration attribute for a user; user or Administrator
    ...
    * -value name             the value of the attribute to set
    */
  def cmd_setUserAttribute(connectionConfig: ConnectionConfig,
                           argMap: Map[String, String]): Unit = {
    val targetUserName: String =
      argMap.getOrElse("-targetUser", argMap.getOrElse("-user", ""))

    if (targetUserName.isEmpty) {
      System.err.println(
        "error: command 'setUserAttribute' requires option '-user' or '-targetUser'")
      System.exit(-1)
    }
    ...
    argMap.get("-key") match {
      case None =>
        System.err.println(
          "error: command 'setUserAttribute' requires option '-key'")
        System.exit(-1)
      ...
      case Some(key) =>
        argMap.get("-value") match {
          case None =>
            System.err.println(
              "error: command 'setUserAttribute' requires option '-value'")
            System.exit(-1)
          ...
          case Some(value) =>
            doAsUser(
              connectionConfig,
              argMap,
              (scalatron: ScalatronRemote, loggedonUser: ScalatronRemote.User,
               users: ScalatronRemote.UserList) => {
                users.get(targetUserName) match {
                  ...
                  case Some(targetUser) =>
                    ...
                    handleScalatronExceptionsFor {
                      ...
                      if (connectionConfig.verbose)
                        println(
                          "Updated attribute '" + key + "' for user '" + targetUserName + "' on the Scalatron server '" + scalatron.hostname + "'")
                    }
                }
              }
            )
  ...
  /** -command getUserAttribute       gets a configuration attribute of a user; user or Administrator
    * -key name               the key of the attribute to set
    */
  def cmd_getUserAttribute(connectionConfig: ConnectionConfig,
                           argMap: Map[String, String]): Unit = {
    val targetUserName: String =
      argMap.getOrElse("-targetUser", argMap.getOrElse("-user", ""))
    ...
    if (targetUserName.isEmpty) {
      System.err.println(
        "error: command 'getUserAttribute' requires option '-user' or '-targetUser'")
      System.exit(-1)
    }
    ...
    argMap.get("-key") match {
      case None =>
        System.err.println(
          "error: command 'getUserAttribute' requires option '-key'")
        System.exit(-1)
                ...
                handleScalatronExceptionsFor {
                  ...
                  attributeMap.get(key) match {
                    case None =>
                      // key is not defined
                      System.err.println(
                        "no value is associated with key '" + key + "' for user '" + targetUserName + "'")
                      System.exit(-2)
                    case Some(value) =>
                      println(value)
                  }
  ...
  /** -command sources         gets a source files from a user workspace; user or Administrator
    * -targetDir path         the path of the local directory where the source files should be stored
    */
  def cmd_sources(connectionConfig: ConnectionConfig,
                  argMap: Map[String, String]): Unit = {
    argMap.get("-targetDir") match {
      case None =>
        System.err.println(
          "error: command 'sources' requires option '-targetDir'")
        System.exit(-1)
          ...
          (scalatron: ScalatronRemote, loggedonUser: ScalatronRemote.User,
           users: ScalatronRemote.UserList) => {
            ...
            handleScalatronExceptionsFor {
              ...
              SourceFileCollection.writeTo(targetDirPath,
                                           sourceFileCollection,
                                           connectionConfig.verbose)

              if (connectionConfig.verbose)
                println(
                  s"Wrote ${sourceFileCollection.size} source files to '$targetDirPath'")
            }
          }
        )
    }
  ...
  /** -command updateSources      updates a source files in the user's server workspace; as user only
    * -sourceDir path         the path of the local directory where the source files can be found
    */
  def cmd_updateSources(connectionConfig: ConnectionConfig,
                        argMap: Map[String, String]): Unit = {
    argMap.get("-sourceDir") match {
      case None =>
        System.err.println(
          "error: command 'updateSources' requires option '-sourceDir'")
        System.exit(-1)
           ...
           users: ScalatronRemote.UserList) => {
            ...
            handleScalatronExceptionsFor {
              val sourceFileCollection =
                SourceFileCollection.loadFrom(sourceDirPath)

              loggedonUser.updateSourceFiles(sourceFileCollection)

              if (connectionConfig.verbose)
                println(
                  s"Updated ${sourceFileCollection.size} source files from '$sourceDirPath'")
            }
          }
        )
    }
  ...
  /** -command build
    */
  def cmd_buildSources(connectionConfig: ConnectionConfig,
                       argMap: Map[String, String]): Unit = {
    doAsUser(
      connectionConfig,
      argMap,
       ...
       users: ScalatronRemote.UserList) => {
        ...
        handleScalatronExceptionsFor {
          ...
          if (buildResult.successful) {
            buildResult.messages.foreach(m =>
              println(
                m.sourceFile + ": line " + m.lineAndColumn._1 + ", col " + m.lineAndColumn._2 + ": " + m.multiLineMessage))
            println(
              s"${buildResult.errorCount} errors, ${buildResult.warningCount} warnings")
          } else {
            buildResult.messages.foreach(m =>
              System.err.println(
                m.sourceFile + ": line " + m.lineAndColumn._1 + ", col " + m.lineAndColumn._2 + ": " + m.multiLineMessage))
  ...
  /** -command publish
    */
  def cmd_publish(connectionConfig: ConnectionConfig,
                  argMap: Map[String, String]): Unit = {
    doAsUser(
      connectionConfig,
      argMap,
  ...
  /** -command versions         gets a list of versions for a specific user; as user only
    */
  def cmd_versions(connectionConfig: ConnectionConfig,
                   argMap: Map[String, String]): Unit = {
    doAsUser(
      connectionConfig,
      argMap,
       ...
       users: ScalatronRemote.UserList) => {
        ...
        handleScalatronExceptionsFor {
          ...
          versionList.foreach(
            v =>
              println(
                "id=\"%s\", label=\"%s\", date=\"%s\"".format(
                  v.id,
                  v.label,
                  DateFormat.getDateTimeInstance.format(new Date(v.date)))))
        }
      }
    )
  }
  ...
  /** -command createVersion      creates a new version in the user's server workspace; as user only
    ...
    * -label string
    */
  def cmd_createVersion(connectionConfig: ConnectionConfig,
                        argMap: Map[String, String]): Unit = {
    argMap.get("-sourceDir") match {
      case None =>
        System.err.println(
          "error: command 'createVersion' requires option '-sourceDir'")
        System.exit(-1)
          ...
          (scalatron: ScalatronRemote, loggedonUser: ScalatronRemote.User,
           users: ScalatronRemote.UserList) => {
            ...
            handleScalatronExceptionsFor {
              ...
              val version =
                loggedonUser.createVersion(label, sourceFileCollection)

              if (connectionConfig.verbose)
                println(
                  s"Create version ${version.id} from ${sourceFileCollection.size} source files from '$sourceDirPath'")
            }
          }
        )
    }
  ...
  /** -command restoreVersion     restores the version with the given ID in the user's workspace and fetches the associated files; as user only
    ...
    */
  def cmd_restoreVersion(connectionConfig: ConnectionConfig,
                         argMap: Map[String, String]): Unit = {
    argMap.get("-targetDir") match {
      case None =>
        System.err.println(
          "error: command 'restoreVersion' requires option '-targetDir'")
        System.exit(-1)
      ...
      case Some(targetDirPath) =>
        argMap.get("-id") match {
          case None =>
            System.err.println(
              "error: command 'restoreVersion' requires option '-id'")
            System.exit(-1)
               ...
               users: ScalatronRemote.UserList) => {
                ...
                handleScalatronExceptionsFor {
                  loggedonUser.version(versionIdStr) match {
                    case None =>
                      System.err.println(
                        s"error: cannot locate version with id '$versionIdStr'")
                      System.exit(-1)
  ...
  /** -command benchmark
    * -sourceDir path     the path of the local directory where the source files can be found
    */
  def cmd_benchmark(connectionConfig: ConnectionConfig,
                    argMap: Map[String, String]): Unit = {
    argMap.get("-sourceDir") match {
      case None =>
        System.err.println(
          "error: command 'benchmark' requires option '-sourceDir'")
        System.exit(-1)
           ...
           users: ScalatronRemote.UserList) => {
            handleScalatronExceptionsFor {
              ...
              for (i <- 0 until 20) {
                ...
                finalSandbox.entities
                  .filter(_.isMaster)
                  .foreach(e => {
                    if (connectionConfig.verbose) {
                      println("Simulated until time = " + finalSandbox.time)
                      println(
                        "Final bot energy score = " + e.mostRecentControlFunctionInput.params
                          .getOrElse("energy", "?"))
                    } else {
                      println(
                        e.mostRecentControlFunctionInput.params
                          .getOrElse("energy", "?"))
                    }
                  })
  ...
  /** -command stresstest     runs a stress test, simulating a hack-a-thon workload on the server; as Administrator only
    * -clients int            the number of clients to simulate (default: 1)
    */
  def cmd_stresstest(connectionConfig: ConnectionConfig,
                     argMap: Map[String, String]): Unit = {
    ...
    def stressTask(clientId: Int): Unit = {
      ...
      try {
        ...
        (0 until Int.MaxValue).foreach(outerCounter => {
          ...
          regularUser
            .logOn(userPassword) // log-on as Administrator (1 round-trip)

          // update the source code to the reference bot
          val sampleList = scalatron.samples
          val referenceBotSample =
            sampleList.get("Example Bot 01 - Reference").get
          ...
          (0 until innerCycleCount).foreach(innerCounter => {
            val (buildResult, buildTime) = timedResult {
              regularUser.buildSources()
            }
            log(
              s"$outerCounter:$innerCounter - buildSources() - $buildTime ms total, ${buildResult.duration} ms pure - ${if (buildResult.successful) "succeeded"
              else s"failed (${buildResult.messages.mkString(",")})"}")

            val (sandbox, sandboxTime) = timedResult {
              regularUser.createSandbox()
            }
          ...
          regularUser
            .logOff() // log off as regular user password (1 round-trip)
        })
      } catch {
        case t: Throwable =>
          log("exception: " + t)
    ...
    (0 until clientCount).foreach(
      n =>
        new Thread(new Runnable { def run(): Unit = { stressTask(n) } },
                   "LoadThread-" + n).start())
  }
  ...
  private def doAsAdministrator(
    ...
    // retrieve Administrator credentials
    val adminUserName =
      argMap.getOrElse("-user", ScalatronRemote.Constants.AdminUserName)
    if (adminUserName != ScalatronRemote.Constants.AdminUserName) {
      System.err.println(
        "error: command requires log-on as user '" + ScalatronRemote.Constants.AdminUserName + "'")
      System.exit(-1)
    }
    val adminPassword = argMap.getOrElse("-password", "")
  ...
  /** Performs the given action by logging on as Administrator, running the command(s), then
    ...
    * @param action the action to perform.
    */
  private def doAsUser(connectionConfig: ConnectionConfig,
                       argMap: Map[String, String],
                       action: (ScalatronRemote, ScalatronRemote.User,
                                ScalatronRemote.UserList) => Unit): Unit = {
    val scalatron = ScalatronRemote(connectionConfig)
