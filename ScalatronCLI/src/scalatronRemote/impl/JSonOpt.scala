...
case class JSonOpt(opt: Option[Any]) {
  ...
  def asMap: JSonMap = opt match {
    case None =>
      throw new IllegalStateException("invalid data (no JSON found)")
    case Some(map: Map[_, _]) =>
      JSonMap(map.asInstanceOf[Map[String, Any]], this)
    case _ =>
      throw new IllegalStateException("invalid data (expected Map): " + opt)
  }
}
...
case class JSonMap(map: Map[String, _], outer: JSonOpt) {
  ...
  def asString(key: String): String = map.get(key) match {
    case None =>
      throw new IllegalStateException(
        "invalid data (key not found: '" + key + "'): " + outer)
    case Some(s: String) => s
    case _ =>
      throw new IllegalStateException(
        "invalid data (value is not a String for key: '" + key + "'): " + outer)
  }
  ...
  def asBoolean(key: String): Boolean = map.get(key) match {
    ...
    case _ =>
      throw new IllegalStateException(
        "invalid data (value is not a Boolean for key: '" + key + "'): " + outer)
  }
  ...
  def asInt(key: String): Int = map.get(key) match {
    ...
    case _ =>
      throw new IllegalStateException(
        "invalid data (value is not an Int for key: '" + key + "'): " + outer)
  }
  ...
  def asList[A](key: String): List[A] = map.get(key) match {
    ...
    case _ =>
      throw new IllegalStateException(
        "invalid data (value is not a List for key: '" + key + "'): " + outer)
  }
  ...
  def asStringMap(key: String): Map[String, String] = map.get(key) match {
    case None =>
      throw new IllegalStateException(
        "invalid data (key not found: '" + key + "'): " + outer)
    case Some(innerMap: Map[_, _]) =>
      innerMap.map(entry => {
        (entry._1.toString, entry._2.asInstanceOf[String])
      })
    case _ =>
      throw new IllegalStateException(
        "invalid data (value is not a Map for key: '" + key + "'): " + outer)
  }
  ...
  /** Retrieves a List value (JSON Array) for the given key that is expected to contain a
    ...
    */
  def asKVStrings(listKey: String,
                  nameKey: String,
                  valueKey: String): Map[String, String] =
    asList[Map[String, String]](listKey).map {
      ...
      case _ =>
        throw new IllegalStateException(
          "invalid data (expected Map): " + outer)
    }.toMap
  ...
  /** Retrieves a List value (JSON Array) for the given key that is expected to contain a
    ...
    */
  def asKVVStrings(listKey: String,
                   nameKey: String,
                   value1Key: String,
                   value2Key: String): Map[String, (String, String)] =
    asList[Map[String, String]](listKey).map {
      case map: Map[String, Any] =>
        val name = map(nameKey) // CBB: use match / case
