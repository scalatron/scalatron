...
case class ScalatronSandboxState(stateData: StateData,
                                 sandbox: ScalatronSandbox)
  ...
  def step(count: Int) = {
    try {
      ...
      val nextResourceUrl =
        s"/api/users/${user.name}/sandboxes/${sandbox.id}/$desiredTime"
      ...
      if (nextStateData.id != stateData.id)
        throw new IllegalStateException(
          s"stepping from state of sandbox id=${sandbox.id} returned state in sandbox with id=${nextStateData.id}")
  ...
  object StateData {
    def fromJson(jsonOpt: JSonOpt) = {
      ...
      val entities = entityList.map(e => {
        ...
        val icMap =
          JSonMap(map.map("input").asInstanceOf[Map[String, Any]], jsonOpt)
        val inputCommand =
          Command(icMap.asString("opcode"), icMap.asStringMap("params"))
        ...
        val outputCommands = outputCommandList.map(c => {
          ...
          Command(cMap.asString("opcode"), cMap.asStringMap("params"))
        })
