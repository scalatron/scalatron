...
case class ScalatronUser(
  ...
  def logOn(password: String): Unit = {
    try {
      val jsonOpt = scalatron.connection
        .POST_json_json(sessionUrl, "{\"password\":\"" + password + "\"}")
    ...
    try {
      val attributesJson =
        "{ \"attributes\" : [ " +
          map
            .map(kv => {
              "{ \"name\" : \"" + kv._1 + "\", \"value\" : " + JSONFormat
                .defaultFormatter(kv._2) + " }"
            })
            .mkString(",\n") +
          " ] }"
  ...
  def createVersion(
      label: String,
      sourceFileCollection: SourceFileCollection): ScalatronVersion = {
    try {
      /*
            {
                "label" : "Label 1",
      ...
      val versionJSon =
        "{ " +
          ...
          "]}"
      val jsonOpt =
        scalatron.connection.POST_json_json(versionsResource, versionJSon)
  ...
  def createSandbox(
      argMap: Map[String, String] = Map.empty): ScalatronSandbox = {
    try {
      /*
            {
                "config" :
      ...
      val sandboxConfigJSon =
        "{ " +
          ...
          "} }"
      val jsonOpt =
        scalatron.connection.POST_json_json(sandboxResource, sandboxConfigJSon)
