...
object build extends Build {
  ...
  lazy val implVersion = Seq(packageOptions <<= (version) map {
    scalatronVersion =>
      Seq(
        Package.ManifestAttributes(("Implementation-Version", scalatronVersion))
      )
  })
  ...
  lazy val cli = Project(
    ...
    ) ++ Seq(jarName in assembly := "ScalatronCLI.jar")
  )
  ...
  lazy val markdown = Project(
    ...
    ) ++ Seq(jarName in assembly := "ScalaMarkdown.jar")
  )

  lazy val samples =
    (IO.listFiles(file("Scalatron") / "samples")) filter (!_.isFile) map {
      sample: File =>
  ...
  val distTask = dist <<= (version, scalaVersion) map {
